...
const coinController = async () => {
  ...
  if (state.coins.allIds.includes(id)) {
    ...
    coinView.renderCoin(
      ...
      state.portfolio.inPortfolio(id)
    );
  } else {
...
window.addEventListener("load", async () => {
  ...
  if (window.location.hash) await coinController();
...
document.querySelector(".converter__link").addEventListener("click", () => {
  ...
  // AMOUNT INPUT
  document
    .querySelector(".amount__to__convert")
    .addEventListener("input", converterController);
  // CONVERTER INPUTS
  [
    ...document.querySelectorAll(".converter__input__a, .converter__input__b"),
  ].forEach((element, i) => {
  ...
  function processInput(type) {
    ...
    const results = state.search.searchCoins(query, true).map((c) => ({
      id: c.id,
      name: c.name,
      symbol: c.symbol,
...
elements.coin.addEventListener("click", () => {
  ...
  if (event.target.matches(".add__portfolio")) {
    const markup = portfolioView.addAmount(
      state.coin,
      state.portfolio.inPortfolio(state.coin.id)
    );
    createPopup(markup);
    document
      .querySelector(".portfolio__input")
      .addEventListener("input", () => {
        const value = parseFloat(event.target.value);
        document.querySelector(".portfolio__submit").disabled =
          value > 0 ? false : true;
...
document.querySelector(".find").addEventListener("click", () => {
  if (document.querySelector(".show__likes")) elements.likesIcon.click();
...
document.addEventListener("keydown", () => {
  ...
  if (event.keyCode === 13 && !state.popup) {
    ...
    if (
      event.target === elements.searchField &&
      document.querySelector(".results__link")
    ) {
  ...
  if (event.keyCode === 13 && state.popup) {
    if (
      event.target.matches(".convert__link") ||
      event.target.matches(".insert__link")
    ) {
      event.preventDefault();
      event.target.click();
    }
    if (
      event.target.matches(
        ".converter__input__a, .converter__input__b, .insert__coin__input"
      )
    ) {
      event.preventDefault();
      if (event.target.nextElementSibling.firstElementChild.innerHTML !== "") {
        const firstResult =
          event.target.nextElementSibling.firstElementChild.firstElementChild
            .firstElementChild;
        firstResult.click();
      }
    }
  ...
  if (event.keyCode === 38) {
    ...
    if (
      ...
      if (event.target.classList.contains("results__link"))
        parentInput = elements.searchField;
      parentInput.focus();
    }
  }
  ...
  if (event.keyCode === 40) {
    ...
    if (
      state.popup &&
      (document.querySelector(".convert__link") ||
        document.querySelector(".insert__link"))
    ) {
      if (
        event.target.matches(
          ".converter__input__a, .converter__input__b, .insert__coin__input"
...
document.querySelector(".content").addEventListener("click", () => {
  ...
  for (const [key, value] of Object.entries(sorts)) {
    if (event.target.matches(`.${key}`)) {
      elements.popup.innerHTML = coinsView.renderAllCoins(
...
document.querySelector(".popup-overlay").addEventListener("click", () => {
  ...
  if (event.target.matches(".insert__portfolio__link")) {
    ...
    document.querySelector(".insert__content").innerHTML =
      portfolioView.insertContent;
    //input field
    document
      .querySelector(".insert__coin__input")
      .addEventListener("input", () => {
  ...
  if (event.target.matches(".insert__link")) {
    ...
    state.portfolio_add_coin.getCoin(
      state.coins.allCoins,
      state.coins.metadata
    );
    // const newItem =
    ...
    document.querySelector(
      ".insert__coin__input"
    ).placeholder = `${state.portfolio_add_coin.name} (${state.portfolio_add_coin.symbol})`;
    document.querySelector(".insert__results__list").innerHTML = "";
    document.querySelector(".insert__results__wrapper").classList.add("hide");

    document.querySelector(".insert__amount").innerHTML =
      portfolioView.addAmount(
    ...
    document
      ...
      .addEventListener("input", () => {
        const value = parseFloat(event.target.value);
        document.querySelector(".insert__submit").disabled =
          value > 0 ? false : true;
        if (event.target.value.length > 10) {
          event.target.value = event.target.value.slice(0, 10);
          if (!parseFloat(event.target.value) > 0)
  ...
  if (event.target.matches(".insert__submit")) {
    ...
    if (state.coin)
      portfolioView.togglePortfolioButton(
        state.coin.id === state.portfolio_add_coin.id
      );
    ...
    const element = elements.popup.querySelector(
      `li[data-id="${state.portfolio_add_coin.id}"]`
    );
  ...
  if (event.target.closest(".portfolio__item")) {
    if (
      !event.target.matches(
        ".delete__icon, .edit__icon, .save__icon, .edit__amount__input"
      )
    ) {
      [
        ...document.querySelectorAll(".edit__amount__input, .save__icon"),
      ].forEach((el) => el.classList.add("hide"));
      [...document.querySelectorAll(".balance__amount, .edit__icon")].forEach(
  ...
  if (event.target.matches(".edit__icon")) {
    const element = event.target.parentElement.parentElement.parentElement;
    const id = parseInt(element.dataset.id);
    [...document.querySelectorAll(".edit__amount__input, .save__icon")].forEach(
    ...
    element
      .querySelector(".edit__amount__input")
      .addEventListener("input", () => {
        const value = parseFloat(event.target.value);
        if (value > 0) {
  ...
  if (event.target.matches(".save__icon, .save__button")) {
    ...
    const newAmount = parseFloat(
      element.querySelector(".edit__amount__input").value
    );
  ...
  [
    ...
    if (event.target.matches(`.${item}`)) {
      elements.popup.innerHTML = portfolioView.renderPortfolio(
  ...
  if (
    !event.target.closest(
      ".results__list__a, .results__list__b, .converter__input__a, .converter__input__b"
    )
  ) {
  ...
  ["a", "b"].map((item) => {
    if (event.target.matches(`.convert__item__${item}`)) {
      const id = parseInt(event.target.dataset.id, 10);
      item === "a"
        ? (state.converter_a = new Converter(id))
        : (state.converter_b = new Converter(id));
      item === "a"
        ? state.converter_a.getValues(state.coins.allCoins)
        : state.converter_b.getValues(state.coins.allCoins);
      converterController();
      ...
      document
        .querySelector(`.results__wrapper__${item}`)
        .classList.add("hide");
    }
    if (event.target.matches(`.converter__usd__${item}`)) {
      item === "a"
        ? (state.converter_a = new Converter())
        : (state.converter_b = new Converter());
      item === "a"
        ? state.converter_a.getValues()
        : state.converter_b.getValues();
      converterController();
    }
  });
  //swap currencies
  if (event.target.matches(".converter__swap")) {
    state.converter_b = {
      ...[state.converter_a, (state.converter_a = { ...state.converter_b })][0],
    };
    converterController();
  }
});
...
elements.notifications.addEventListener("click", () => {
  ...
  if (event.target.matches(".watchlist")) {
    ...
    const id = parseInt(event.target.dataset.id, 10);
    const element = elements.likesPanel.querySelector(
      `a[href="#${id}"]`
    ).parentElement;
    element.classList.add("updated");
    element.scrollIntoView();
...
document.querySelector(".day-night").addEventListener("click", () => {
  elements.html.classList.contains("dark")
    ? setDayNight("day")
    : setDayNight("night");
});
